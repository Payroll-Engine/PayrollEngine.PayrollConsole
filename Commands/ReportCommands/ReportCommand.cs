...
internal sealed class ReportCommand : CommandBase<ReportParameters>
{
        ...
        try
        {
            ...
            switch (parameters.DocumentType)
            {
                ...
                case DocumentType.Pdf:
                    var mergeParameters = new Dictionary<string, object>(
                        reportParameters.Select(x => new KeyValuePair<string, object>(x.Key, x.Value)));
    ...
    private async Task<ReportResponse> ExecuteReport(ICommandConsole console,
        ...
        ReportRequest request, ReportSet report)
    {
        var response = await new ReportService(httpClient).ExecuteReportAsync(
            context: new(tenantId, regulationId),
            reportId: report.Id,
            request: request);
        if (response == null)
        {
        ...
        ReportParameters parameters, Dictionary<string, object> mergeParameters)
    {
        ...
        {
            // report template
            var template = GetReportTemplate(report, parameters.Culture);
            if (template == null)
            {
        ...
        DataSet dataSet, string culture, string targetFile = null)
    {
        ...
        var docXml = new UTF8Encoding().GetString(stream.ToArray());
    ...
    //private static XmlDocument DataSetToXml(DataSet dataSet)
    //{
    //    using MemoryStream stream = new();
    //    dataSet.WriteXml(stream, XmlWriteMode.WriteSchema);
